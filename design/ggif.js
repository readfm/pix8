window.Ggif = {
	...
	send: function(){
		...
		var hash = Ggif.newHash || randomString(4);
		var stream = bin.send(buf, {
			hash: hash,
			name: hash+'.gif',
			mime: 'image/gif',
			size: buf.byteLength
        ...
        stream.on('data', function(r){
			if(r.file){
				...
				ws.send({
					...
					if(!r.item) return $('#gif-save').blink('red');
					console.log(r.item);

					var $item = pix.build(r.item);
	...
	glueUpText: function(color){
		...
		$('#frames > canvas').each(function(){
			...
			ctx.wrapText(text, 4, 4, canvas.width, 15);
	...
	// get text shown in twext are with upper case where required
	getUpText: function(time){
		var text = '', up = false, prev = 0;
		time
		$('#game > i').each(function(i){
			var $i = $(this),
				tm = $i.data('time');
