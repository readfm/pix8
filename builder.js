window.Builder = {
	...
	item: function(item, mod){
		...
		var file = item.file;
		if(!file && item.src){
			if(item.src.indexOf(Cfg.files) === 0)
				file = item.src.substr(Cfg.files.length);
		}
	...
	gPictures: {},
	getGPicture: (gName, cb) => {
		var url = Builder.gPictures[gName];
		if(url) return cb(url);
		else if(url === false) return;

		$.getJSON('http://picasaweb.google.com/data/entry/api/user/'+gName+'?alt=json', r => {
			if(!r || !r.entry) return;
